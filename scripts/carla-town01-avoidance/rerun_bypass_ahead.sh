...
PED_X=$(python3 -c "print(f'{float(\"${ego_x:-47}\")+14.0:.2f}')")
PED_Y=-3.60
...
timeout 5 ros2 param set "$BPP" avoidance.target_filtering.parked_vehicle.th_offset_from_centerline 0.4 >/dev/null 2>&1
timeout 5 ros2 param set "$MVP" obstacle_stop.obstacle_filtering.check_outside.default false >/dev/null 2>&1
timeout 5 ros2 param set "$MVP" obstacle_stop.stop_planning.stop_margin 4.0 >/dev/null 2>&1
...
timeout 50 ros2 topic pub -r 10 /planning/scenario_planning/max_velocity tier4_planning_msgs/msg/VelocityLimit \
  "{max_velocity: 5.56, use_constraints: false, sender: ahead}" >/dev/null 2>&1 &

bash /home/aw/aw_docker/force_engage.sh

echo "[ahead] sample 30s — expect LEFT shift (y increases toward 0) before ped"
